...
class TreeNode:
    def __init__(self):
        self.children = []
        self.group_key = None
        self.public_key = None  # Store public key for the node

    def add_member_TreeNode(self, new_public_key):
        """Add member node with existing public key"""
        new_node = TreeNode()
        new_node.public_key = new_public_key  # Use provided public key
        self.children.append(new_node)
        self.update_key_TreeNode()

    def remove_member_TreeNode(self, public_key_to_remove):
        """Remove member node by public key"""
    ...
    def update_key_TreeNode(self):
        """Update group key based on current tree structure"""
        self.group_key = self.generate_group_key_TreeNode()

    def generate_group_key_TreeNode(self):
        """Generate group key from children's keys"""
        if not self.children:
            return os.urandom(32)
        combined_key = b"".join(
            child.generate_group_key_TreeNode() for child in self.children
        )
        return HKDF(
    ...
    def print_tree(self, level=0):
        ...
        if not self.children:
            ...
            print(
                "  " * level + f"Node Level {level}: Group Key: {self.group_key.hex()}"
            )
        for child in self.children:
